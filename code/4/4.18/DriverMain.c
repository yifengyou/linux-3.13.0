...
{
	long *pBigObject, *pSmallObject;
	struct page *pBigObjectPage, *pSmallObjectPage;
	unsigned long mask;
	...
	pBigObject = (long *)kmalloc(9*1024, 0);
	pSmallObject = (long *)kmalloc(8, 0);

	pBigObjectPage = virt_to_page(pBigObject);
	pSmallObjectPage = virt_to_page(pSmallObject);

	mask = 1 << PG_slab;

	if(pBigObjectPage->flags & mask)
	{
		DEBUG_PRINT(DEVICE_NAME " BigObject sets PG_slab\n");
	}
	else
	{
		DEBUG_PRINT(DEVICE_NAME " BigObject does not set PG_slab\n");
	}

	if(pSmallObjectPage->flags & mask)
	{
		DEBUG_PRINT(DEVICE_NAME " SmallObject sets PG_slab\n");
	}
	else
	{
		DEBUG_PRINT(DEVICE_NAME " SmallObject does not set PG_slab\n");
	}
