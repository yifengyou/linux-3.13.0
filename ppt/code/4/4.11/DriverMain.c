...
void PrintActiveSlab(struct kmem_cache *pKMemCache)
{
	...
	DEBUG_PRINT(DEVICE_NAME " Active slab page frame no = 0x%lx\n", (unsigned long)page_to_pfn(pPage));
	DEBUG_PRINT(DEVICE_NAME " Active slab freelist = 0x%lx\n", (unsigned long)pCache->freelist);

	DEBUG_PRINT(DEVICE_NAME " Active slab page->freelist = 0x%lx\n", (unsigned long)pPage->freelist);
	DEBUG_PRINT(DEVICE_NAME " Active slab pPage->inuse = 0x%lx\n", (unsigned long)pPage->inuse);
	DEBUG_PRINT(DEVICE_NAME " Active slab pPage->objects = 0x%lx\n", (unsigned long)pPage->objects);
	DEBUG_PRINT(DEVICE_NAME " Active slab pPage->frozen = 0x%lx\n", (unsigned long)pPage->frozen);
	DEBUG_PRINT(DEVICE_NAME " Active slab pPage->slab_cache = 0x%lx\n", (unsigned long)pPage->slab_cache);
	DEBUG_PRINT(DEVICE_NAME " Active slab pKMemCache = 0x%lx\n", (unsigned long)pKMemCache);
...
static int DriverInitialize(void)
{
	...
	PrintActiveSlab(kmalloc_caches[6]);
